...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/vic_larlite_play/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/vic_larlite_play/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/vic_larlite_play/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/vic_larlite_play/doc/doxygenMyProject.script
